...
step is the first deploy, which creates the `docs.siftline.dev` custom domain and its
...
That pin has a second, deliberate consequence: `apps/docs` is not checked by TypeScript 7.
Its `typecheck` script is `tsc --noEmit`, and the nearest `tsc` to `apps/docs` is the one
its own `typescript@6.0.3` installs, so the docs app is typechecked by the JavaScript
...
change needs no release, record that deliberately with `bun changeset --empty`
