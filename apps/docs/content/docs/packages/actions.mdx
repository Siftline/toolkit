...
`fetch` defaults to the global one, read on each call. The option is typed narrower than the
global `fetch`, so the global one and a stub both fit without a cast. `signal` goes to `fetch`
unchanged.
