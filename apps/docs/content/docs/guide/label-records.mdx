...
A Record is one message with an id. Its `state` is what the model reads, and it can be a
...
Look at `msg-3`. The model leaned towards `question` at 0.58, which is under the Recipe's
threshold of 0.7, so the Decision carries `review: true`. It is still written, with its
