...
Ten words cover the whole toolkit. Each one is a type or a file format in `@siftline/core`, so
...
nothing else about it: sender, source and raw payload stay with you.
...
it, so you can always tell which questions a result answered.
...
`@siftline/actions` builds the request.
